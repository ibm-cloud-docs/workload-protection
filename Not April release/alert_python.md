...
You can manage alerts in a {{site.data.keyword.sysdigsecure_full}} instance by using the {{site.data.keyword.sysdigsecure_full_notm}} Python client.
{: shortdesc}

To learn how to use the Python client, see [Using the Python client](/docs/workload-protection?topic=workload-protection-python-client).
...
# Add the instance information that is required for authentication
URL = <ENDPOINT>
APIKEY = <IAM_APIKEY>
GUID = <GUID>
...
* You must include the following information: `<ENDPOINT>`, `<IAM_APIKEY>`, and `<GUID>` This data is required to authenticate the request with the {{site.data.keyword.sysdigsecure_full_notm}}  instance. To get the instance information, see [Authenticate your user or service ID by using IAM](/docs/workload-protection?topic=workload-protection-python-client#python-client-iam-auth).
...
* You must define the notification channels through which you want to be notified when the alert is triggered.
    ...
    When you define the notification channels, the channels must be configured in the {{site.data.keyword.sysdigsecure_full_notm}} instance.
...
* [`condition`]: You must define the condition that defines when the alert is triggered. For example, you can set this parameter to `['host.mac', 'proc.name']` to check a CPU alert for every process on every machine.
    ...
    For more information, see [Multi-Condition Alerts](/docs/workload-protection?topic=workload-protection-notifications).
